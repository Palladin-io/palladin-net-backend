...
| `DomainWriteContextBase` | `Palladin.Core.Persistence/DomainWriteContextBase.cs` | Exposes tracked aggregate roots and, after `SaveChangesAsync`, dispatches accumulated domain events via `HandleEventAsync` | Each module's `XxxDomainWriteContext` extends it; mutating features call `CommitAsync` |
