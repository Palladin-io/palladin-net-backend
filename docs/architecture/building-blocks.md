...
| `AllowNonActiveOrganizationMembershipAttribute` | `Palladin.Core.Security/AllowNonActiveOrganizationMembershipAttribute.cs` | Marks one reviewed exception to the API's default active-membership boundary | Apply only to an account/exit operation or semantically read-only POST that must remain available during staged offboarding; every use is protected by an exact architecture-test allowlist |
| `RequireActiveOrganizationMembershipAttribute` | `Palladin.Core.Security/RequireActiveOrganizationMembershipAttribute.cs` | Opts a safe-verb endpoint into the authoritative active-membership check that unsafe user mutations receive by default | Apply to sensitive GET endpoints whose identifiers or state must not remain visible during staged offboarding |
| `RequireAssignableOrganizationRoleAttribute` | `Palladin.Core.Security/RequireAssignableOrganizationRoleAttribute.cs` | Marks a role mutation that must be authorized against Identity's current role snapshot and records that exact source revision for the feature's local-replica fence | Apply to cross-module role mutations; the feature must reject a missing or mismatched authorized revision rather than trusting an eventually consistent role replica |
...
The API attaches `RequireActiveOrganizationMembershipPreProcessor` (`src/Palladin.Api/Framework/`) to every endpoint by default. Safe HTTP methods remain readable during staged offboarding. Every unsafe method carrying the complete user-JWT claim set (`sub`, `org_id`, `authz_ver`) requires the current membership to be `Active`; anonymous, agent and system principals are outside this user-JWT boundary. Exceptions must use `AllowNonActiveOrganizationMembershipAttribute` on the endpoint class—never path strings or constructor/DI reflection—and must be added to the exact allowlist in `ActiveOrganizationMembershipBoundaryArchitectureTests` after review. The current allowlist is limited to Identity account/exit flows, Notification read/preferences/push registration, and the read-only Vault member snapshot/delta and Search GlobalSearch POSTs.

`RequireAssignableOrganizationRolePreProcessor` is an API-composition boundary, not an eventual-consistency shortcut. It reads the current Identity role and active caller membership, allows the Owner to target `Administrator`, rejects `Administrator` for every non-owner, and otherwise rejects roles above the caller's current effective permission ceiling. It passes the authoritative role revision, caller ceiling and Owner status through request-local state. A marked Vault mutation proceeds only when its local role-directory revision is exactly the authorized revision. Later full-state role events are monotonic; an elevation beyond the recorded ceiling invalidates the existing policy, supersedes its operation and reconciles the desired selection without ever creating or changing cryptographic membership. System-role status alone does not invalidate a policy.

Typed current-state dispatch rows are deliberately narrower than a generic outbox. Identity uses them for full role and member-role-set replicas; Vault uses them for full role-Vault policy state. The row contains tenant and aggregate identifiers, a monotonic source revision, the complete non-secret structural state, a deterministic key, publish progress, bounded-backoff retry metadata, and no key material or plaintext Vault metadata. Publishers retry indefinitely with a capped delay; Identity also republishes acknowledged current snapshots on a bounded repair interval so a consumer failure that exhausts broker retries cannot become permanent. Consumers compare source revisions, making anti-entropy duplicates and out-of-order delivery safe.
