...
| `DomainWriteContextBase.MarkPropertyAsUpdated` | `Palladin.Core.Persistence/DomainWriteContextBase.cs` | Forces a property write while preserving tracked changes and the original concurrency-token snapshot | Use for a no-op optimistic authority fence in the same aggregate commit. It attaches detached objects only; reattaching a tracked modified entity would erase pending changes or replace the original concurrency values |
