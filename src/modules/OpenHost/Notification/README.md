...
- Identity's `login-lockout-alert` transactional template is sent only for a verified known account. It presents the three-attempt/one-minute threshold, the source IP, UTC detection/expiry times and the fact that only that `(account, IP)` pair is temporarily blocked. It deliberately has no unlock button or token; the 15-minute block expires automatically. Identity supplies an account/time-bucket idempotency key so repeated deliveries and lockouts from additional IPs cannot amplify e-mail within the same aligned lockout interval.
