...
- Commands for one-time e-mails must carry a deterministic `IdempotencyKey`. Notification persists a short optimistic `Dispatching` lease before SES, calls the provider without a database transaction, then marks the delivery `Sent`. A provider failure removes the owned claim so MassTransit can retry; an expired claim can be reclaimed after a process crash. SES has no application idempotency token, so a crash after provider acceptance but before `Sent` is durably recorded has an explicit at-least-once duplicate window.
