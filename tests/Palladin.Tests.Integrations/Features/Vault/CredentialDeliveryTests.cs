...
public sealed class CredentialDeliveryTests(ApiFactory apiFactory) : TestBase
{
    ...
    private static GrantEntryScope Material(Guid orgId, Guid vaultId, Guid grantId, Guid entryId, int? remainingUses = null) =>
        GrantEnvelopeTestData.Scope(orgId, vaultId, grantId, entryId, remainingUses: remainingUses);
